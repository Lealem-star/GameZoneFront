...
function useCountUp(target, duration = 10) {
...
const GameControllerDetail = () => {
    ...
    // Calculate total revenue for today only (% system revenue)
    ...
    const totalRevenueToday = todayGames.reduce(
        (sum, game) => sum + ((Number(game.entranceFee) || 0) * (Array.isArray(game.participants) ? game.participants.length : 0) * 0),
        0
    );
            ...
            <div className="flex-1 flex flex-col justify-center">
                <div className="w-full px-0 sm:px-8 py-8">
                    <div className="flex justify-between items-start mb-6 w-full animate-fade-in-down">
                        {/* Profile Card */}
                        <div className="flex items-center gap-6 bg-gray-50 rounded-lg p-4 border border-gray-0 w-fit animate-fade-in-left">
                            <div className="w-24 h-24 rounded-full overflow-hidden bg-gray-0 flex items-center justify-center">
                                {controller.image ? (
                                    <img src={`${controller.image}`} alt={controller.username} className="w-full h-full object-cover" />
                                ) : (
                        ...
                        {/* Back Button */}
                        <button onClick={() => navigate(-1)} className="px-6 py-2 bg-gray-0 rounded-lg hover:bg-gray-300 font-semibold border border-gray-300 shadow-sm animate-fade-in-up">Back</button>
                    </div>
                    {/* Total Revenue Card */}
                    <div className="flex justify-center mb-8 w-full animate-fade-in-up">
                        <div className="bg-gradient-to-br from-blue-100 to-blue-0 rounded-xl shadow-lg px-8 py-6 text-lg font-semibold border border-blue-0 w-full max-w-2xl text-center flex flex-col items-center">
                    ...
                    {/* Games Table */}
                    <div className="overflow-x-auto border-t border-gray-0 pt-4 w-full animate-fade-in-up delay-0">
                        <table className="min-w-full bg-white rounded-xl shadow-lg text-sm animate-fade-in-up">
                            <thead>
                                <tr className="bg-gradient-to-r from-yellow-100 to-orange-100 border-b border-gray-0">
                            ...
                            <tbody>
                                {sortedGames.length === 0 ? (
                                    ...
                                    sortedGames.map(game => (
                                        <tr key={game._id} className="hover:bg-yellow-50 border-b border-gray-100 transition-all duration-0 animate-fade-in-up">
                                            ...
                                            <td className="px-4 py-2 font-semibold text-blue-700">{((Number(game.entranceFee) || 0) * (Array.isArray(game.participants) ? game.participants.length : 0) * 0.6).toFixed(2)} ETB</td>
                                            <td className="px-4 py-2 font-semibold text-green-700">{((Number(game.entranceFee) || 0) * (Array.isArray(game.participants) ? game.participants.length : 0) * 0.4).toFixed(2)} ETB</td>
                                        </tr>
                                    ))
                                )}
                            </tbody>
